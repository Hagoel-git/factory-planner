...
// --- Fix for ImGui 1.92+ compatibility ---
static inline void ImRect_Floor(ImRect& r) 
{ 
    r.Min.x = IM_FLOOR(r.Min.x); 
    r.Min.y = IM_FLOOR(r.Min.y); 
    r.Max.x = IM_FLOOR(r.Max.x); 
    r.Max.y = IM_FLOOR(r.Max.y); 
}
// -----------------------------------------
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
    ...
    if (!Editor->GetConfig().DisableInternalFitView && Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(GetKeyIndexForF()) && Editor->AreShortcutsEnabled())
    {
